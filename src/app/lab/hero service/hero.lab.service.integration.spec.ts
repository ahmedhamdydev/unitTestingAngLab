...
describe('3-hero service (http) integration testing:', () => {
  ...
  it('getHeroes function: send request and receive response successfully', () => {
    const mockHero: Hero = { id: 3, name: 'spider man', strength: 11 };

    service.getHero(3).subscribe((hero) => {
      expect(hero.name).toBe('spider man');
    });

    const testRequest = httpTesting.expectOne(`${heroesUrl}/3`);
    expect(testRequest.request.method).toBe('GET');
    testRequest.flush(mockHero);
  });
